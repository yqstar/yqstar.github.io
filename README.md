...
│   │   └── papers.css         # 论文阅读页样式
│   └── js/
│       ├── navigation.js      # 首页与阅读页共用的四入口顶部导航
...
根目录只保留首页 `index.html`，子页面归入 `pages/`，其中四个面试专题统一放在 `pages/interviews/`。样式、交互和学习内容分别放在 `assets/css/`、`assets/js/` 和 `data/`。首页、论文页和面试页共用顶部导航，提供概览、算法、AI 专题和论文四个入口；面试页文件名不变，同一站点的浏览器题库继续沿用原存储键。
...
站点统一使用「算法训练场」名称，顶部单行放置返回 Study Hub 首页（`../index.html`）、工具和主题切换。默认题面展开、代码模式和学习记录备份集中到“工具”，统计收进“学习概况”；继续练习、随机题和搜索筛选保持直接可用。题目与评测逻辑沿用源文件，Python 加载增加浏览器兼容适配；历史存储键和 JSON 备份格式保持兼容，已有代码、笔记和进度在同一站点来源下继续可用。
...
同步脚本校验来源文件，替换站点展示名称，加入 `scripts/algorithms-site.js` 中的导航和布局适配，并内嵌共享主题脚本和 `scripts/algorithms-runtime.js` 中的启动适配，直接写入 `pages/algorithms.html`，不压缩或添加解压启动页。题库、编辑器、学习记录存储和 Python 评测逻辑保留源文件实现，运行时不依赖外部脚本。修改布局、共享主题或运行时适配后也需重新运行同步命令。升级上游版本时先检查文案、导航、主题和运行时适配点，再一起更新提交号与校验值。
