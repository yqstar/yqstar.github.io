...
`pages/papers.html` 收录 8 篇经典 AI 论文，覆盖模型架构、高效微调、偏好对齐、Agent 与检索四个方向。每篇提供原始来源与阅读导引，从研究问题、核心方法、实验结果及局限组织阅读；可搜索标题和作者、按方向或阅读状态筛选，并记录进度与个人笔记。
...
维护论文内容时编辑 `data/papers.js`；页面行为和外观分别位于 `assets/js/papers.js` 与 `assets/css/papers.css`。增加或替换论文时应核对标题、作者、年份和来源链接，并同步首页的论文数量。

运行 `node scripts/check-papers.mjs` 可检查筛选、笔记保存、备份导入导出、存储不可用时的行为、页面导航及桌面和窄屏的深浅色布局。检查使用临时 Chrome 配置，不读取个人浏览器记录；`CHROME_PATH` 可指定浏览器安装位置。
