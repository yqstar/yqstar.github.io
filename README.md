...
原生 HTML/CSS/JavaScript 静态学习站，包含 100 道 Python 算法练习、Transformer、RL、SFT、Agent 的 114 道精选面试题，以及 39 篇中文论文全文和 12 篇经典 AI 与推荐论文解析。无需安装依赖或构建即可运行。
...
│       └── papers.js          # 论文筛选、学习导航与历史备份
├── data/
│   ├── papers.js              # 51 篇论文资料、来源与译文阅读导引
│   ├── paper-translations-manifest.json # PDF 来源文件名、页数、大小与 SHA256
│   ├── paper-analyses.js      # 12 篇中文解析、实验数据与来源链接
│   ├── paper-lessons.js       # 51 篇讲义、102 道自测与学习路线
│   └── interviews/
│       ├── transformer.js
│       ├── rl.js
│       ├── sft.js
...
通过合集按钮可切换到12 篇经典解析，或浏览全部 51 篇。经典解析保留结论速览、流程图、方法拆解、核心公式、教学示例、实验对照、贡献、局限和原文定位；教学例与论文实验明确区分。全部论文均提供进阶讲义。

新增 DIN、DIEN、SASRec、BERT4Rec 四篇推荐基础解析，含来源明确的实验对照。全部 51 篇均增加“推导与动手学习”：先备知识、学习目标、分步推导、手算反例、实验方案和 102 道可展开答案的自测。其中 4 篇提供仅依赖 Python 标准库的最小算例。教学算例和拟议实验不代表论文实验或已经完成的复现。

页首“按路线系统学习”提供序列到生成式推荐、特征交互到统一排序主干、基础模型到适配与偏好学习三条路线。路线与“接着读”链接可跨合集打开目标论文；深读讲义支持 `#paper=tiger&section=study` 直接定位。所有 39 篇译文均保留阅读导引、中文全文，并补齐进阶讲义。

讲义与路线维护在 `data/paper-lessons.js`，新增推荐解析在 `data/paper-analyses.js`。新增实验数字需核对来源版本、数据切分和负采样；例如 SASRec 与 BERT4Rec 原文的同名指标不能跨论文直接相减。

可搜索论文标题、作者和解析全文，通过“筛选”按方向查找。每篇底部的笔记编辑和阅读状态区已移除，学习目录不再显示笔记入口。“阅读工具”仅保留历史笔记的只读导出，不再提供导入、编辑或进度同步。

旧版 `study-hub:papers:v1` 记录不会被迁移、删除或改写。历史导出在点击时读取最新原始内容；没有记录或存储不可用时给出提示，不影响阅读功能。英文原文链接需要联网打开。

论文数据集中在三个普通脚本中：`papers.js` 维护清单，`paper-analyses.js` 维护经典解析，`paper-lessons.js` 维护讲义与路线。无需打包器，也支持直接通过 `file://` 打开。删除了分批追加数据与跨文件修改清单的加载依赖。

基础资料位于 `data/papers.js`，解析正文位于 `data/paper-analyses.js`，按同一论文 ID 对应。解析须核对全文与实验表，注明测试/开发集、基线及数值口径，不能把教学示例写成论文结果。页面行为和外观分别位于 `assets/js/papers.js` 与 `assets/css/papers.css`。增加或替换论文时应核对标题、作者、年份和来源链接，并同步首页的论文数量。

译文元数据位于 `data/papers.js`，中文 PDF 位于 `assets/papers/zh/`。来源清单 `data/paper-translations-manifest.json` 记录原始目录内的相对路径、文件大小、页数与 SHA256，便于核对复制结果；不包含英文 PDF、LaTeX 构建文件或本机工具链。替换译文时须同步清单与页面元数据。少数原文网页已更新标题或实验数字，导引以所收录的本地译稿对应版本为依据。

运行 `node scripts/check-papers.mjs` 可检查 39 份中文 PDF 与清单的一致性、英文来源链接、合集与关键词筛选、51 篇进阶讲义与 102 道自测、笔记区移除与历史记录保留、12 篇经典解析、目录跳转与焦点、历史备份只读导出、空记录和存储不可用时的行为、页面导航及桌面和窄屏的深浅色布局。若本机存在原始译文目录，还会逐一比对来源文件；可用 `ARXIV_PAPER_DIR` 指定目录。检查使用临时 Chrome 配置，不读取个人浏览器记录；`CHROME_PATH` 可指定浏览器安装位置。
